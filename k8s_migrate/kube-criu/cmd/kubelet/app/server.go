...
(underspec'd currently) to submit a new manifest.`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err := Run(kubeletServer, kubeletDeps, stopCh, kubeconfigFile); err != nil {
				glog.Fatal(err)
			}
		},
	}
...
// not be generated.
func Run(s *options.KubeletServer, kubeDeps *kubelet.Dependencies, stopCh <-chan struct{}, configFile string) error {
	// To help debugging, immediately log version
	glog.Infof("Version: %+v", version.Get())
	if err := initForOS(s.KubeletFlags.WindowsService); err != nil {
		return fmt.Errorf("failed OS init: %v", err)
	}
	if err := run(s, kubeDeps, stopCh, configFile); err != nil {
...
func run(s *options.KubeletServer, kubeDeps *kubelet.Dependencies, stopCh <-chan struct{}, configFile string) (err error) {
