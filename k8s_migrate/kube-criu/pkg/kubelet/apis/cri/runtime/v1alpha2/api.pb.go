...
func (*StartContainerFromCheckpointRequest) Descriptor() ([]byte, []int) {
	return fileDescriptorApi, []int{102}
}

type CheckpointContainerRequest struct {
	// ID of the container to start.
	ContainerId string `protobuf:"bytes,1,opt,name=container_id,json=containerId,proto3" json:"containerid,omitempty"`
	// ID of the checkpoint
	Checkpoint string `protobuf:"bytes,2,opt,name=checkpoint,json=checkpoint,proto3" json:"checkpointid,omitempty"`
	// CheckpointDir
	CheckpointDir string `protobuf:"bytes,3,opt,name=checkpoint_dir,json=checkpointDir,proto3" json:"checkpointdir,omitempty"`
	// PreDump
	PreDump bool `protobuf:"bytes,4,opt,name=predump,json=predump,proto3" json:"preDump,omitempty"`
}
...
func (*CheckpointContainerRequest) Descriptor() ([]byte, []int) {
	return fileDescriptorApi, []int{103}
}
...
func (c *runtimeServiceClient) CheckpointContainer(ctx context.Context, in *CheckpointContainerRequest, opts ...grpc.CallOption) (*StartContainerResponse, error) {
	out := new(StartContainerResponse)
	err := grpc.Invoke(ctx, "/runtime.v1alpha2.RuntimeService/CheckpointContainer", in, out, c.cc, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
type RuntimeServiceServer interface {
	...
	CheckpointContainer(ctx context.Context, in *CheckpointContainerRequest) (*StartContainerResponse, error)
	// StopContainer stops a running container with a grace period (i.e., timeout).
	// This call is idempotent, and must not return an error if the container has
	// already been stopped.
	// TODO: what must the runtime do after the grace period is reached?
