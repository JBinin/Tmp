...
func (m *manager) Start() {
	...
	go wait.Forever(func() {
		...
		case syncCheckpointRequest := <-m.podCheckpointStatusChannel:
			glog.V(5).Infof("Status Manager: syncing podcheckpoint: %q, with status: (%d, %v) from podCheckpointStatusChannel",
				syncCheckpointRequest.podCheckpointUID, syncCheckpointRequest.status.version, syncCheckpointRequest.status.status)
			glog.Warningf("Status Manager: syncing podcheckpoint: %q, with status: (%d, %v) from podCheckpointStatusChannel",
				syncCheckpointRequest.podCheckpointUID, syncCheckpointRequest.status.version, syncCheckpointRequest.status.status)
...
func (m *manager) SetPodCheckpointStatus(podcheckpoint *v1alpha1.PodCheckpoint, status v1alpha1.PodCheckpointStatus) {
	...
	// Make sure we're caching a deep copy.
	status = *status.DeepCopy()
	glog.Warningf("setpodcheckpointstatus, the status is %q\n", status)
...
func (m *manager) updatePodCheckpointStatusInternal(podcheckpoint *v1alpha1.PodCheckpoint, status v1alpha1.PodCheckpointStatus, forceUpdate bool) bool {
	...
	case m.podCheckpointStatusChannel <- podCheckpointStatusSyncRequest{podcheckpoint.UID, newStatus}:
		glog.V(5).Infof("Status Manager: adding podcheckpoint: %q, with status: (%q, %v) to podCheckpointStatusChannel",
...
func (m *manager) syncPodCheckpoint(uid types.UID, status versionedPodCheckpointStatus) {
	glog.Warningf("the status is %q\n", status)
	// TODO: make me easier to express from client code
	podcheckpoint, err := m.podcheckpointClient.PodcheckpointcontrollerV1alpha1().PodCheckpoints(status.podCheckpointNamespace).Get(status.podCheckpointName, metav1.GetOptions{})
	glog.Warningf("the podcheckpoint is %q\n", podcheckpoint)
	if errors.IsNotFound(err) {
		glog.V(3).Infof("Podcheckpoint %q (%s) does not exist on the server", status.podCheckpointName, uid)
		// If the Pod is deleted the status will be cleared in
		// RemoveOrphanedStatuses, so we just ignore the update here.
		return
	}
	if err != nil {
		glog.Warningf("Failed to get status for podcheckpoint %q: %v", format.PodDesc(status.podCheckpointName, status.podCheckpointNamespace, uid), err)
		return
	}

	// oldStatus := podcheckpoint.Status.DeepCopy()
	podcheckpoint.Status = status.status
	glog.Warningf("the podcheckpoint status is %q\n", podcheckpoint.Status)
	newPodCheckpoint, err := m.podcheckpointClient.PodcheckpointcontrollerV1alpha1().PodCheckpoints(podcheckpoint.Namespace).Update(podcheckpoint)
	podcheckpoint_new, err := m.podcheckpointClient.PodcheckpointcontrollerV1alpha1().PodCheckpoints(status.podCheckpointNamespace).Get(status.podCheckpointName, metav1.GetOptions{})
	glog.Warningf("the podcheckpoint_new is %q\n", podcheckpoint_new)
	if err != nil {
		...
		glog.Warningf("the newPodCheckpoint is %q\n", newPodCheckpoint)
		glog.Warningf("the PodCheckpoint status is %q, newpodcheckpointstatus is %q\n", podcheckpoint.Status, newPodCheckpoint.Status)
		return
	}

	glog.V(0).Infof("Status for podcheckpoint %q updated successfully: (%d, %+v)", fmt.Sprintf("%s_%s(%s)", podcheckpoint.Name, podcheckpoint.Namespace, podcheckpoint.UID), status.version, status.status)
	glog.V(0).Infof("Status for newpodcheckpoint %v", newPodCheckpoint.Status)
