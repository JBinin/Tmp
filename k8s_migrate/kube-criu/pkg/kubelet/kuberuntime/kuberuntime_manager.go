...
import (
	...
	v1alpha "k8s.io/podcheckpoint/pkg/apis/podcheckpointcontroller/v1alpha1"
...
func (m *kubeGenericRuntimeManager) CheckpointPod(pod *v1.Pod, podcheckpoint *v1alpha.PodCheckpoint, secret *v1.Secret) {
	...
	for {
		if preDump {
			...
			for i := 0; i < len(pod.Status.ContainerStatuses); i++ {
				...
				podcheckpoint.Status.ContainerConditions[i].CheckpointPhase = v1alpha.ContainerCheckpointing
				dump, err := m.checkpointContainer(pod, &(pod.Status.ContainerStatuses[i]), podcheckpoint, secret, preDump, iterCount)
				if err != nil {
					glog.Infof("Checkpoint %q round for container %s failed", iterCount, pod.Status.ContainerStatuses[i].Name)
				}
	...
	glog.Infof("Final Dump!!!")
	for i := 0; i < len(pod.Status.ContainerStatuses); i++ {
		fmt.Printf("traverse containerStatus : %v", pod.Status.ContainerStatuses[i])
		podcheckpoint.Status.ContainerConditions[i].CheckpointPhase = v1alpha.ContainerCheckpointing
		_, err := m.checkpointContainer(pod, &(pod.Status.ContainerStatuses[i]), podcheckpoint, secret, preDump, iterCount)
		if err != nil {
			podcheckpoint.Status.ContainerConditions[i].CheckpointPhase = v1alpha.ContainerCheckpointFailed
			failed_once = true
			continue
		}
		podcheckpoint.Status.ContainerConditions[i].CheckpointPhase = v1alpha.ContainerCheckpointSucceeded
		succeeded_once = true
	}

	if succeeded_once && !failed_once {
		// Succeed to Checkpoint Pod
		podcheckpoint.Status.Phase = v1alpha.PodSucceeded
	} else if succeeded_once && failed_once {
		// Some Container in Pod Fail to Checkpoint
		podcheckpoint.Status.Phase = v1alpha.PodHalfFailed
	} else {
		// All Container in Pod Fail to Checkpoint
		podcheckpoint.Status.Phase = v1alpha.PodFailed
	}
...
func (m *kubeGenericRuntimeManager) SyncPod(pod *v1.Pod, _ v1.PodStatus, podStatus *kubecontainer.PodStatus, pullSecrets []v1.Secret, backOff *flowcontrol.Backoff, ossSecret *v1.Secret) (result kubecontainer.PodSyncResult) {
	...
	if container := podContainerChanges.NextInitContainerToStart; container != nil {
		...
		if msg, err := m.startContainer(podSandboxID, podSandboxConfig, container, pod, podStatus, pullSecrets, podIP, kubecontainer.ContainerTypeInit, ossSecret); err != nil {
			startContainerResult.Fail(err, msg)
	...
	for _, idx := range podContainerChanges.ContainersToStart {
		...
		if msg, err := m.startContainer(podSandboxID, podSandboxConfig, container, pod, podStatus, pullSecrets, podIP, kubecontainer.ContainerTypeRegular, ossSecret); err != nil {
			startContainerResult.Fail(err, msg)
