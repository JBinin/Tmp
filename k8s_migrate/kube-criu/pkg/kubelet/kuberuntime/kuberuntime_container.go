...
import (
	...
	dumpstats "k8s.io/podcheckpoint/dumpstats"
)
...
func (m *kubeGenericRuntimeManager) checkpointContainer(pod *v1.Pod, containerStatus *v1.ContainerStatus, podcheckpoint *v1alpha1.PodCheckpoint, ossSecret *v1.Secret, preDump bool, iterCount int) (bool, error) {
	if containerStatus.ContainerID[0:9] == "docker://" {
		...
		if preDump {
			...
			if err != nil {
				glog.Infof("Some Error Happened in CheckPreCopy!!!")
				return false, err
			}
			return preCopy, nil
		} else {
			...
			glog.Infof("complete upload, clear checkpoint file; container name is:",containerStatus.Name)
...
func (m *kubeGenericRuntimeManager) startContainer(podSandboxID string, podSandboxConfig *runtimeapi.PodSandboxConfig, container *v1.Container, pod *v1.Pod, podStatus *kubecontainer.PodStatus, pullSecrets []v1.Secret, podIP string, containerType kubecontainer.ContainerType, ossSecret *v1.Secret) (string, error) {
	...
	if podCheckpointName := pod.Annotations["podCheckpoint"]; podCheckpointName != "" {
		podcheckpoint, err := m.podcheckpointClient.PodcheckpointcontrollerV1alpha1().PodCheckpoints(pod.Namespace).Get(podCheckpointName, metav1.GetOptions{})
		...
		if podcheckpoint.Status.Phase == v1alpha1.PodSucceeded {
			...
			glog.Warningf("podcheckpoint status is %s\n", podcheckpoint.Status.Phase)
			glog.Warningf("podcheckpoint %s(_%s) Checkpoint Failed,Pod can't restore from podcheckpoint %s", podCheckpointName, podcheckpoint.Namespace, podCheckpointName, err)
			return "", kubecontainer.ErrRunContainer
		}
	} else {
