...
func makePodSourceConfig(kubeCfg *kubeletconfiginternal.KubeletConfiguration, kubeDeps *Dependencies, nodeName types.NodeName, bootstrapCheckpointPath string, configFile string) (*config.PodConfig, error) {
	...
	if kubeDeps.KubeClient != nil {
		...
		config.NewSourceApiserver(kubeDeps.KubeClient, nodeName, updatechannel, configFile)
	}
	return cfg, nil
}
...
func NewMainKubelet(kubeCfg *kubeletconfiginternal.KubeletConfiguration,
	...
	if kubeDeps.PodConfig == nil {
		var err error
		kubeDeps.PodConfig, err = makePodSourceConfig(kubeCfg, kubeDeps, nodeName, bootstrapCheckpointPath, configFile)
		if err != nil {
			return nil, err
		}
	}
	...
	if err != nil {
		glog.Fatalf("Error building podcheckpoint clientset: %s", err.Error())
	}
...
func (kl *Kubelet) syncPod(o syncPodOptions) error {
	...
	if podCheckpointName := pod.Annotations["podCheckpoint"]; podCheckpointName != "" {
		podcheckpoint, err := kl.podCheckpointClient.PodcheckpointcontrollerV1alpha1().PodCheckpoints(pod.Namespace).Get(podCheckpointName, metav1.GetOptions{})
		if apierrors.IsNotFound(err) {
			glog.V(3).Infof("Podcheckpoint %q does not exist on the server", podCheckpointName)
			// If the Pod is deleted the status will be cleared in
			// RemoveOrphanedStatuses, so we just ignore the update here.
			return err
		}
		if err != nil {
			glog.Warningf("Failed to get podcheckpoint %s", podCheckpointName, err)
			return err
		}
		ossSecretName := podcheckpoint.Spec.SecretName
		Secret, err := kl.kubeClient.CoreV1().Secrets(o.pod.Namespace).Get(ossSecretName, metav1.GetOptions{})
		ossSecret = Secret
		if err != nil {
			fmt.Println("The Ossak hasn't been Set Correctlly! Error:%v",err)
		}
	}	
	
	if updateType == kubetypes.SyncPodCheckpoint {
		o.podcheckpoint.Status.Phase = v1alpha1.PodCheckpointing
		fmt.Println("Entering SetPodcheckpointStatus firstly, the status is:%s\n",o.podcheckpoint.Status.Phase)
		kl.statusManager.SetPodCheckpointStatus(o.podcheckpoint, o.podcheckpoint.Status)
		fmt.Println("Finished SetPodcheckpointStatus firstly and entering checkpoint, the status is:%s\n",o.podcheckpoint.Status.Phase)
		kl.containerRuntime.CheckpointPod(pod, o.podcheckpoint, ossSecret)
		fmt.Println("Finished checkpoint and entering SetPodCheckpointStatus secondly, the status is:%s\n",o.podcheckpoint.Status.Phase)
		kl.statusManager.SetPodCheckpointStatus(o.podcheckpoint, o.podcheckpoint.Status)
		fmt.Println("Finished SetPodCheckpointStatus secondly, the status is:%s\n",o.podcheckpoint.Status.Phase)
		return nil
	}
	...
	// Call the container runtime's SyncPod callback
	result := kl.containerRuntime.SyncPod(pod, apiPodStatus, podStatus, pullSecrets, kl.backOff, ossSecret)
