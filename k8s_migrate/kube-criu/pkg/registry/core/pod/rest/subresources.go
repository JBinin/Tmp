...
// DumpREST implements the dump subresource for a Pod
...
// Implement Connecter
var _ = rest.Connecter(&DumpREST{})

// New creates a new podDumpOptions object.
func (r *DumpREST) New() runtime.Object {
	return &api.PodDumpOptions{}
}

// Connect returns a handler for the pod dump proxy
func (r *DumpREST) Connect(ctx context.Context, name string, opts runtime.Object, responder rest.Responder) (http.Handler, error) {
	dumpOpts, ok := opts.(*api.PodDumpOptions)
	if !ok {
		return nil, fmt.Errorf("invalid options object: %#v", opts)
	}
	location, transport, err := pod.DumpLocation(r.Store, r.KubeletConn, ctx, name, dumpOpts)
	if err != nil {
		return nil, err
	}
	return newThrottledUpgradeAwareProxyHandler(location, transport, false, true, true, responder), nil
...
// NewConnectOptions returns the versioned object that represents dump parameters
func (r *DumpREST) NewConnectOptions() (runtime.Object, bool, string) {
	return &api.PodDumpOptions{}, false, ""
}

// ConnectMethods returns the methods supported by dump
func (r *DumpREST) ConnectMethods() []string {
	return upgradeableMethods
}
